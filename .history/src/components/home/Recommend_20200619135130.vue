    ...
    <div ref="wrapper" class="contAll">
      ...
      <div class="cont">
        ...
        <div class="cont-item" v-for="(item,index) in recommend" :key="index">
          <div>
            <img class="img" :src="item.image" alt />  <!-- 推荐商品图片 -->
          </div>
          <div class="cont-name">{{item.goodsName}}</div> <!-- 推荐商品名称 -->
          <div class="priceAll">
            <div>￥{{item.mallPrice}}</div>
            <div class="price">￥{{item.price}}</div>  <!-- 推荐商品价格 -->
          </div>

          <div class="btn">
            <div class="car"><van-icon name="cart" color='white'/></div>  <!-- 推荐商品加购 -->
            <div class="see">查看详情</div>  <!-- 推荐商品详情 -->
          </div>
        </div>
      </div>
    </div>
