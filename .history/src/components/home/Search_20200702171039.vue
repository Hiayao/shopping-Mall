<template>
  ...
  <div>
    <div class="list">
      <div v-if="this.value !== ''"> <!-- 判断搜索词不为空时显示搜索的商品，为空时显示历史搜索词 -->
        <div v-if="this.list.length > 0"> <!-- 判断搜索词有对应商品时显示商品名字，没有时显示无 -->
      ...
      <div v-else>
        ...
        <div class="arrAll" v-if="this.arr.length > 0"><!-- 判断有历史记录时显示历史关键词，否则显示无 -->
...
export default {
  ...
  methods: {
    ...
    //       Toast.success('删除成功')
    //     }
    //     localStorage.setItem("search", JSON.stringify(this.list));
    //   }
    // },
    // 删除所有记录关键词
    delAll(){
    ...
    delItem(item){
      ...
      for (let i = 0; i < this.arr.length; i++) {
        ...
        localStorage.setItem("search", JSON.parse(this.arr));
      }
            // on confirm 点击确定按钮后做什么
          })
          .catch(() => {
