<template>
  ...
  <van-swipe-item v-for="(item, index) in slides" :key="index">
    <img v-lazy="item.image" class="img" @click="gotoDetails"/>
  </van-swipe-item>
</van-swipe>
  </div>
</template>
...
export default {
  ...
  methods: {
    gotoDetails(){
      ...
      console.log(this.$store.state.slides[item].goodsId)
      this.$router.push({path:'/details',query:{id:this.$store.state.slides[item].goodsId}})
    }
  },
  mounted() {
    
    console.log(this.$store.state.slides[index].goodsId)
  },
  watch: {},
  computed: {
    slides() {
