<template>
  ...
  <div>
    ...
    <div v-if="this.list[0] !==2">
      <div class="news" v-if="this.add !== null">
        <div>
      ...
      <div class="news" v-else-if="this.ids !== null">
        <div>
          <van-icon name="location-o" size="30" />
        </div>
        <div>
          <div class="person">
            <div>收货人：{{this.ids.name}}</div>
            <div>{{this.ids.tel}}</div>
          </div>
          <div class="person">收货地址：{{this.ids.address}}</div>
          <div class="ps">（收货不便时，可选择免费接待收货服务）</div>
        </div>
        <div>
          <van-icon name="arrow" size="20" @click="gotoAdd" />
    ...
    <div v-else class="news">
      ...
      <div>
        <div class="person">
          <div>收货人：{{this.list[2].name}}</div>
          <div>{{this.list[2].tel}}</div>
        </div>
        <div class="person">收货地址：{{this.list[2].address}}</div>
        <div class="ps">（收货不便时，可选择免费接待收货服务）</div>
      </div>
      <div>
        <van-icon name="arrow" size="20" @click="gotoAdd" />
    ...
    <div class="shoplist">
      <!-- 从购物车购买 -->
      <div v-if="this.list[1] === false && this.list.length > 0">
