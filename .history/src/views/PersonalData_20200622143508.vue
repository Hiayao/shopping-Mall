...
export default {
  ...
  methods: {
    ...
    save(){
        this.$api.saveUser({gender:this.userInfo.gender,year:this.year,month:this.month,day:this.day,nickname:this.userInfo.nickname}).then(res => {
            console.log(res)
        }).catch(err => {
            console.log(err)
        })
  ...
  mounted() {
    this.$api
      ...
      .then(res => {
        ...
        this.value = this.userInfo.year + '年' + this.userInfo.month + '月' + this.userInfo.day + '日' ;
        console.log(res);
      })
      .catch(err => {
        console.log(err);
