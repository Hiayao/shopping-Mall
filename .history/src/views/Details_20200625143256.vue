<template>
  ...
  <div class="detail_all">
    ...
    <div class="content">
      ...
      <div class="like" v-if="flag === false " @click="sure">   <!-- 收藏与取消 -->
        <div>收藏：</div>
        <div>
...
<!-- 弹框内商品的名称和价格 -->
          <div class="ptONE">
            ...
            <div style="color:red">￥ {{goodsOne.present_price.toFixed(2)}}</div>
          </div>
        </div>
        <van-divider />
<!-- 弹框内商品购买数量 -->
        ...
        <div class="num2">
          ...
          <div class="Stepper"><van-stepper v-model="value" min="1" max="50" integer/></div>
        </div>
         <van-divider />
<!-- 弹框内操作购买按钮 -->
         <van-button type="danger" block class="nowbuy">立即购买</van-button>
      </van-popup>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    addCart(){
      this.$api.addShop(this.goodsId).then(res => {
        console.log(res)
      }).catch(err =>{
        console.log(err)
      })
