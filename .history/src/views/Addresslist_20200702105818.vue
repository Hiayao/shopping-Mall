<template>
  ...
  <div>
    <div class="top">
      ...
      <van-nav-bar title="地址列表" left-text left-arrow @click-left="onClickLeft" v-if="this.fleg">
      <van-nav-bar title="地址列表" left-text left-arrow @click-left="onClickLeft" v-if="!this.fleg">
        <template #right>
...
export default {
  ...
  methods: {
    ...
    onClickLeft() {
      ...
      // if (this.fleg === true) {
      //   this.$router.push({ path: "/settlement", query: { id: [2, true] } });
      // }
      // // 当跳转到此页面没有传参过来时
      // if (this.fleg === undefined) {
      //   this.$router.go(-1);
      //   Toast("返回");
      // }
    },
...
// 当从购物车购买到结算跳转地址列表时传参为false，返回传参为false
      if (this.fleg === false) {
        this.$router.push({ path: "/settlement", query: { id: [2, false,item] } });
      localStorage.setItem("ids", JSON.stringify(item));
      }
      // 当从直接购买到结算跳转地址列表时传参为true，返回传参为true
      if (this.fleg === true) {
        this.$router.push({ path: "/settlement", query: { id: [2, true,item] } });
      localStorage.setItem("ids", JSON.stringify(item));
      }
