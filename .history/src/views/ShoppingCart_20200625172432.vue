<template>
  ...
  <div>
    ...
    <div class="tip">
      <div class="sure">
        <div class="check">
          <van-checkbox v-model="checked" shape="square" checked-color="red" @click="checkAll">全选</van-checkbox>
        </div>
        <div class="total">
          <div>合计：</div>
          <div>请确认订单</div>
      ...
      <div class="btn">
        <div @click="del">
          <van-button color="skyblue" v-model="XX">删除</van-button>
        </div>
    ...
    <div class="list">
      <div class="shoplist" v-for="(item,index) in shoplist" :key="index">
        <div class="check">
          <van-checkbox v-model="item.checke" shape="square" checked-color="red"></van-checkbox>
        </div>
        <div class="details">
          <div>
...
export default {
  ...
  methods: {
      checkAll(){
          this.checke = !this.checked
          this.shoplist.map(item => {
         item.checke = this.checked
      })
      },
      del(){
          this.$api.deleteShop(id).then(res => {
              console.log(res)
          }).catch(err => {
              console.log(err)
          })
